...
int main(int argc, char** argv)
{
	...
	const char* keys =
		{
			"{ f folder   | ex2.avi | folder of video            }"
	...
	std::string folder = cmd.get<std::string>("folder");
...
//	LOG(INFO)<<"Starting extraction";
  ...
  std::cout << "Lets start computing from " << folder << std::endl;
	calcDenseFlow(folder, bound, type, 1,
